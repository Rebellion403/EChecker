          ...
          if [[ "$get" = "" ]]
           then  
                ...
                if [[ "$hash" = "y" ]];then
                   ...
                   echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}] ${G}LIVE => $email$del$pass ${N}|${G} Acc $asu ${N}|${G} Checked by Rebellion"
                fi
           fi
        fi
     done
...
if [[ "$hash" = "y" ]];then
   ...
   echo -e "[*] Terimakasih sudah menggunakan tols ini:')"
else
   echo -e "-------[ checked by Rebellion checker ]--------\n" >> $acc-$list
   echo -e "${N}[*] Done.....\n[+] Result : ${G}$acc-$list${N}"
   ...
   echo -e "${G}- Terimakasih telah memakai tols ini:')${N}"
fi
}
 echo -e "

"Author: Rebellion ${m}v.1.0${p}"
  "Team: UNDERGROUND team"
...
case $d in
 ...
 ;;
 10) echo -e "
         |${G} Underground Team${N}|

Thx to:${O} GiovaIT- MhankBarBar - Underfif - Cr0w - Bakwan.Id - Scoby - QNZURA - Rens - Guitar_Cracker_Cyber - Rexy - DamnIT" 
